  ...
  function px2rem(file, encoding, callback) {
    ...
    array_style.forEach(function (value) {
      if (ignore_selector.indexOf(value.split('{')[0].replace(/(^\s*)|(\s*$)/g, '')) > -1) {} else {
        value = value.replace(/\s+[0]\s+|((\s*)(-?)(\d+)(rem+))/g, function (word) {

          var rem_num = parseFloat(word);
          ignore_px.push(0);
          if (ignore_px.indexOf(rem_num) > -1) return word;

          //var rem_num = rem_num / width_design * pieces;

          var rem_num = rem_num * width_design;
          return ' ' + new Number(rem_num).toFixed(valid_num) + 'px';
        });
      }
